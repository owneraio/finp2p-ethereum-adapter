...
function registerDirectServices(
  ...
  if (appConfig.accountModel === 'omnibus') {
    ...
    const directPaymentsService = new DirectPaymentsServiceImpl(distributionService, appConfig, custodyProvider);
    register(app, tokenService, escrowService, commonService, commonService, directPaymentsService, planApprovalService, pluginManager, workflowsConfig, undefined, mappingService);
    return;
  }
...
async function createApp(
  ...
  switch (appConfig.type) {
    case 'fireblocks': {
      const custodyProvider = await FireblocksCustodyProvider.create(appConfig, logger);
