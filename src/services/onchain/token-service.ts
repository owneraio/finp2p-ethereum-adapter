...
export class OnChainTokenService implements TokenService, EscrowService, CommonService, HealthService {
   ...
   * polls the final result. Completes with the single receipt of this
  ...
  public async swap(idempotencyKey: string, nonce: string, operationId: string, assetLeg: SwapLeg,
    ...
    try {
      ...
      const executedHere = this.hasSwapExecuted(txReceipt);
  ...
  private hasSwapExecuted(txReceipt: { logs: readonly { topics: readonly string[], data: string }[] }): boolean {
    return txReceipt.logs.some((log) => {
      try {
  ...
  /**
   * Single-call swap (both wallets custodied here): the operator contract has no
   * single-tx both-legs primitive, so drive its two-party prepare/execute mirror
  ...
  public async swapSingle(idempotencyKey: string, nonce: string, operationId: string, asset: SwapLeg,
    ...
    try {
      ...
      // 1st call — the asset owner's perspective: records its leg (SwapPrepared);
      // an operationId prepared out of band may already execute here
      const prepareTx = await this.finP2PContract.swap(operationId, {
        assetId: asset.asset.assetId,
        assetFinId: asset.source.finId,
        assetAmount: asset.quantity,
        settlementAssetId: settlement.asset.assetId,
        settlementFinId: settlement.source.finId,
        settlementAmount: settlement.quantity,
      });

      let executedTx = prepareTx;
      if (!this.hasSwapExecuted(prepareTx)) {
        // 2nd call — the settlement owner's mirrored perspective: crosses both
        // movements in one tx
        executedTx = await this.finP2PContract.swap(operationId, {
          assetId: settlement.asset.assetId,
          assetFinId: settlement.source.finId,
          assetAmount: settlement.quantity,
          settlementAssetId: asset.asset.assetId,
          settlementFinId: asset.source.finId,
          settlementAmount: asset.quantity,
        });
        if (!this.hasSwapExecuted(executedTx)) {
          return failedSwapSingleOperation(1, `swapSingle ${operationId}: mirror call ${executedTx.hash} did not execute the swap prepared by ${prepareTx.hash}`);
        }
      }

      const transactionId = executedTx.hash;
      const timestamp = (await executedTx.getBlock()).timestamp;
      if (exCtx) {
        this.execDetailsStore?.addExecutionContext(transactionId, exCtx.planId, exCtx.sequence);
      }
