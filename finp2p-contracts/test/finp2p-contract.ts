...
describe("FinP2P proxy contract test", function() {
  ...
  describe("FinP2PProxy operations", () => {
    ...
    const terms: {decimals: number, amount: number}[] = [ // [decimals, issueAmount, transferAmount, redeemAmount]
        {decimals: 0, amount: 10},
        {decimals: 2, amount: 10.13},
        {decimals: 4, amount: 10.0001},
        {decimals: 18, amount: 1.01},
    ];
    primaryTypes.forEach((primaryType) => {
      legs.forEach((leg) => {
        terms.forEach(({decimals, amount}) => {
          it(`issue/transfer/redeem operations (decimals: ${decimals}, amount: ${amount}`, async () => {
            ...
            await contract.issue(issuerFinId, term(assetId, 'finp2p', `${amount.toFixed(decimals)}`), { from: operator });

            expect(await contract.getBalance(assetId, issueBuyerFinId)).to.equal(`${(0).toFixed(decimals)}`);
            expect(await contract.getBalance(assetId, issuerFinId)).to.equal(`${amount.toFixed(decimals)}`);
            ...
            const asset = term(assetId, 'finp2p', `${amount.toFixed(decimals)}`);
            ...
            expect(await contract.getBalance(assetId, buyerFinId)).to.equal(`${amount.toFixed(decimals)}`);

            // ----------------------------------------------------------

            await contract.redeem(buyerFinId, term(assetId, 'finp2p', `${amount.toFixed(decimals)}`), { from: operator });
            expect(await contract.getBalance(assetId, buyerFinId)).to.equal(`${(0).toFixed(decimals)}`);
          });


          it(`hold/release operations (decimals: ${decimals}, amount: ${amount}, primaryType: ${primaryType}, leg: ${leg})`, async () => {
            ...
            const settlement = term(settlementAsset, 'fiat', `${amount.toFixed(decimals)}`);
            ...
            switch (leg) {
              ...
              case Leg.Settlement:
                ...
                await contract.issue(issuerFinId, term(settlementAsset, 'fiat',  `${amount.toFixed(decimals)}`), { from: operator });
                expect(await contract.getBalance(settlementAsset, issuerFinId)).to.equal(`${amount.toFixed(decimals)}`);
                break
              default:
            ...
            switch (leg) {
              ...
              case Leg.Settlement: {
                expect(await contract.getBalance(settlementAsset, buyerFinId)).to.equal(`${(0).toFixed(decimals)}`);
                const lock = await contract.getLockInfo(operationId);
                expect(lock[0]).to.equal(settlementAsset);
                expect(lock[1]).to.equal('fiat');
                expect(lock[2]).to.equal(buyerFinId);
                expect(lock[3]).to.equal(`${amount.toFixed(decimals)}`);
                break
              }
            }
            ...
            await contract.release(operationId, sellerFinId, `${amount.toFixed(decimals)}`, { from: operator });
            switch (leg) {
              case Leg.Asset: {
                expect(await contract.getBalance(assetId, sellerFinId)).to.equal(`${assetAmount.toFixed(decimals)}`);
                expect(await contract.getBalance(assetId, buyerFinId)).to.equal(`${(0).toFixed(decimals)}`);
              ...
              case Leg.Settlement: {
                expect(await contract.getBalance(settlementAsset, sellerFinId)).to.equal(`${amount.toFixed(decimals)}`);
                expect(await contract.getBalance(settlementAsset, buyerFinId)).to.equal(`${(0).toFixed(decimals)}`);
                break
              }
            }
          ...
          it(`hold/redeem operations (decimals: ${decimals}, amount: ${amount})`, async () => {
            const assetId = `bank-us:102:${uuid()}`;
            ...
            await contract.issue(investorFinId, term(assetId, 'finp2p', `${amount.toFixed(decimals)}`), { from: operator });
            expect(await contract.getBalance(assetId, investorFinId)).to.equal(`${amount.toFixed(decimals)}`);
