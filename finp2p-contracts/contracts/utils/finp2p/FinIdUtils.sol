...
library FinIdUtils {
    ...
    function toAddress(string memory finId) internal pure returns (address) {
        return _compressedPublicKeyToAddress(_fromHex(finId));
    }

    function _fromHex(string memory s) public pure returns (bytes memory) {
    ...
    function _compressedPublicKeyToAddress(bytes memory _pubKey) public pure returns (address) {
        uint8 prefix = uint8(_pubKey[0]);
        uint x;
        assembly {
            x := mload(add(_pubKey, add(0x20, 0x01)))
        }
        uint256 y = _deriveY(prefix, x, AA, BB, PP);
        bytes32 hash = keccak256(abi.encodePacked(x, y));
        return address(uint160(uint256(hash)));
    }

    function _deriveY(uint8 _prefix, uint256 _x, uint256 _aa, uint256 _bb, uint256 _pp) internal pure returns (uint256) {
    ...
    function _expmod(uint256 _base, uint256 _exp, uint256 _pp) internal pure returns (uint256) {
