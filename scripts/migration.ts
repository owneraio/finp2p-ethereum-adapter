...
const startMigration = async (ossUrl: string, providerType: ProviderType, finp2pContractAddress: string, grantMinter: boolean, grantOperator: boolean) => {
      ...
      if (foundAddress === tokenAddress) {
        ...
        if (grantMinter) {
          ...
          try {
            ...
            logger.error(`Failed to grant minter role for ${tokenAddress}: ${e}`);
          }
        }
        if (grantOperator) {
          const erc20 = new ERC20Contract(provider, signer, tokenAddress, logger);
          try {
            ...
            logger.error(`Failed to grant operator role for ${tokenAddress}: ${e}`);
          }
        }
        skipped++;
        continue;
...
startMigration(ossUrl, providerType, contractAddress, grantMinter, grantOperator).then(() => {
});
